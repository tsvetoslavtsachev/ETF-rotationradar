...
- [x] ~~Category-rotation-over-time heat-strip (10 cats × ~26 weeks)~~ — **S20 (2026-06-19)**: панел „🔥 Ротация по сектори" в Rotation Radar таба (9 категории × 26 W-FRI седмици; клетка = медианен momentum-перцентил на категорията за седмицата). **Метрика = `unadj_percentile`** (глобален абс. momentum), НЕ `percentile_rank` — последното е ранг ВЪТРЕ в категорията → медиана структурно ~50 (измерено 51.6–62.5) → плосък strip; `unadj` дава контраст 10.3–92.7. Дивержентна скала център 50 (дефиниционни прагове, не измислени). W-FRI resample взима последната снимка на всяка седмица (CI дневни append-и иначе тежат 5×). `src/heatstrip.py::compute_heatstrip` (нула нови вызови) → `render.py` `heatstrip=` → `payload["heatstrip"]`; `app.js::renderHeatstrip` (heatColor дивержентна HSL). smoke step 14. **Затваря целия Tier 3 + ETF роудмапа.**
...
### Optional hardening — ✅ DONE (S15 v2, 2026-06-21)
- ~~FRED keyless `fredgraph.csv` is fragile (06-07 outage). INIT-22 lesson: official `api.stlouisfed.org` + `FRED_API_KEY` is more robust.~~ **DONE:** `src/fred.py` now uses the official API + `FRED_API_KEY` as **primary** (keyless fredgraph → labeled fallback → stale cache). Hardens all 7 routine FRED series at once (breakeven + HY + 5 macro). CI workflow `daily_update.yml` gets the `FRED_API_KEY` secret. Live proof: T10YIE 5870 pts in 0.63s, fresh (06-18=2.25) vs keyless bot-stall. `_parse_api_json` is a pure, smoke-tested parser.

## S15 v2 — flows v2 (real shares) + FRED hardening + barometer contract (2026-06-21, ✅ SHIPPED — commit 7b5f2a6)
Затваря ETF backend-а (последната реална дупка преди S17). Решенията на Цветослав (AskUserQuestion): flows v2 = **yfinance дялове за цялата вселена**; FRED = **FRED_API_KEY primary за целия fred.py**; старо ETF-Dashboard = **пенсиониране в S17, не пипай сега**.
- **Flows v2 — БРОИМ ДЯЛОВЕТЕ, не AUM.** `поток = чиста_цена × Δshares-outstanding`. Дяловете се менят само при creation/redemption → нула ценова информация → лекува одитната находка „flows = чист ценови ефект" ОТ КОРЕН (v1 AUM-проксито е алгебрично същото, но `totalAssets` е стар reported номер при чужд NAV → замърсено). `shares_out` идва от **СЪЩИЯ `.info` вызов** като AUM (`fundamentals.py`, нула нови заявки). Forward-accumulate в `data/shares_history.parquet` (като aum_history; няма backfill — `get_shares_full` връща празно, проверено). v1 AUM-проксито остава ПАРАЛЕЛНО за крос-чек (`compute_flows_aum`; daily_update логва % съгласие по посока). Жива верига доказана: SPY 917M дялове → +3% = +$20.6B приток, GLD −2% = −$2.0B отток. **Резолюция седмична** (същия 7-дн `.info` кеш) — но без ценово замърсяване. Validation watch (като v1): реална оценка щом shares_history натрупа ~21 търг. дни.
- **Barometer contract** — feed=10 ↔ behavioral-tracker v2.0=10 (вече в синхрон от 12→10); нов smoke contract assert (`barometer_feed.json readings == INDICATORS == 10`) затваря открития „12 vs 11" thread от Фаза-1 одита.
- **Старо ETF-Dashboard** — НЕ пипано (решение: retire в S17; ETF-rr е надмножество 131 vs 88 + барометър/макро/бета/COT/movers/heatstrip; преди retire: пренасочи macro-satellite от etfs.json към ETF-rr data.json = S17 retire-стъпка).
- TIP/IEF проксито беше retired още в S15 v1 (barometer 12→10) — само keyless fredgraph остана за решаване → стана label-нат fallback.
- Verify зелени: smoke 16 стъпки PASS (+flows v2 shares · +FRED parser · +contract) · daily_update EXIT=0 (BE=2.25 свежо) · preview :8137 (Flow% нов tooltip, 0 console грешки) · CI run 27912116865 SUCCESS (breakeven 2.25 свежо в CI = secret работи). FRED_API_KEY repo secret сложен (gh, съществуващ ключ — ⚠️ ротацията от 16.06 ОСТАВА препоръчана).
