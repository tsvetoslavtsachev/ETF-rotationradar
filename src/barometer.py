...
ALARM_CONF = 2   # ≥2 едновременни alarm-сензора → регионна тревога. Честотата е РЕЖИМНО-
                 # ЗАВИСИМА: ~5% (trailing-2г) → 17% (3г) → 22% (5г). Одит 07.07.2026: това е
                 # СЪВПАДАЩ стрес-клъстер (капитулационен), НЕ изпреварващо предупреждение —
                 # 87% фалшиво-поз. за спад>−5%; силните аларми паднаха на дъна (апр'25, март'26).
...
def _trend_4w(series: "pd.Series | None") -> "tuple[str, float | None]":
    ...
    средно на последните 2 петъка спрямо средно на първите 2 от 4-те), не 4-седмичен.
    Прагът ±2% е еднакъв за нива (VIX) и спредове → етикетите не са съизмерими между тях."""
    if series is None:
        return "flat", None
    s = series.dropna()
    if len(s) < 2:
